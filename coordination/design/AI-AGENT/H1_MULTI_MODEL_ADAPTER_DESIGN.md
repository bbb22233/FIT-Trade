...
- 基线 commit：`8fcdb82db970a1e2925b53b61c4245f9b597a2ef`
...
| **UNAVAILABLE** | 持有当前发现纪元的完整运行时能力收据，收据中 result 字段证明该能力不可用或被拒绝 | 排除于当前纪元，禁止路由和重试；下一发现纪元必须重新验证 |

### 2.2 当前模型声明（文档级观察，非运行时验证）

> ⚠️ **本文档编写于文档阶段，未执行实时能力发现。以下所有条目均缺少完整运行时能力收据（缺少 timestamp、result、failure_class、evidence_hash 四个由运行时 API 调用方可填充的必需字段；provider、model、capability 的文档级记录同样不是运行时收据），全部为 UNVERIFIED。没有完整、可审计、无密钥的能力收据即必须为 UNVERIFIED。**
>
> **运行时启动时（第 6 节强制流程）必须执行实时能力发现，获得至少一个 VERIFIED 条目后才能建立路由表。**

| # | 模型 ID | 提供商 | 路径 | 状态 | 缺失字段 | 观察来源 | 说明 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `deepseek-v4-pro` | DeepSeek | OpenRouter → DeepSeek | **UNVERIFIED** | timestamp, result, failure_class, evidence_hash | Hermes Agent Memory | 推理模型；max_tokens≥4096；不支持 response_format json_object；content 可能为空、推理内容在 reasoning_content |
| 2 | `llama-4-maverick` | Meta | OpenRouter → Meta | **UNVERIFIED** | timestamp, result, failure_class, evidence_hash | Hermes Agent Memory | 通用对话模型；标准 chat completions API |
| 3 | `mistral-large` | Mistral | OpenRouter → Mistral | **UNVERIFIED** | timestamp, result, failure_class, evidence_hash | Hermes Agent Memory | 通用对话模型；标准 chat completions API |
| 4 | `gpt-5.6-sol` | OpenAI | OpenAI 直连 | **UNVERIFIED** | timestamp, result, failure_class, evidence_hash | Hermes Agent Memory | OpenRouter Provider Allowlist 拦截（403）；OAuth 凭证与 Hermes 不共享 |
| 5 | `gpt-5.6-terra` | OpenAI | OpenAI 直连 | **UNVERIFIED** | timestamp, result, failure_class, evidence_hash | Hermes Agent Memory | 同上 |
| 6 | Anthropic 全系 | Anthropic | OpenRouter → Anthropic | **UNVERIFIED** | 完整运行时能力收据 | — | 历史 Provider Allowlist 观察（403）不属于当前发现纪元的运行时能力收据；不可成为排除于强制发现的理由 |
| 7 | Google 全系 | Google | OpenRouter → Google | **UNVERIFIED** | 完整运行时能力收据 | — | 同上；历史 Provider Allowlist 观察不可替代当前纪元运行时收据 |
...
4. UNAVAILABLE 条目仅当持有当前发现纪元的完整运行时能力收据、收据中 result 字段证明该能力不可用或被拒绝时方可成立。该条目排除于当前纪元的全部路由路径，不消耗重试预算；下一发现纪元必须重新验证
5. 历史 Provider Allowlist 观察、静态配置、文档记忆、过去 session 中记录的 provider 支持状态均不得作为排除 provider 于强制发现的依据
...
- 回退目标模型必须同样完成 §6 强制能力发现，不可使用 UNVERIFIED 条目
...
1. **启动时强制运行**：Hermes Agent Runtime 启动后，在建立路由表之前，必须对 §2.2 中列出的所有 UNVERIFIED 候选模型执行完整能力发现流程。UNAVAILABLE 状态仅在持有当前发现纪元的完整运行时能力收据（证明不可用或被拒绝）时方可使用，不得基于历史观察或静态配置将任何 provider/model 排除于发现范围之外
...
Hermes Agent 启动
  ...
  │     │     └─ 失败 → 生成能力收据 → UNAVAILABLE(当前纪元)
  │     │
  │     ├─ gpt-5.6-sol / gpt-5.6-terra / Anthropic / Google →
  │     │     ├─ 每个 provider/model 都执行 ping 调用；历史 Allowlist 观察不构成排除理由
  │     │     ├─ 成功 → VERIFIED
  │     │     └─ 失败 → 生成能力收据(含 failure_class) → UNAVAILABLE(当前纪元)
  │
  ├─ 3. 汇总结果：
...
1. VERIFIED 和 UNAVAILABLE 收据均带 TTL（默认 300s）。过期后条目自动降级为 UNVERIFIED，并重新进入强制发现
2. 过期 VERIFIED 在任何路由请求前必须重新验证；重新验证最长等待 10s，超时则 fail-closed（NO_TRADE）
3. 过期 UNAVAILABLE 必须回归 UNVERIFIED；不得因上一纪元的 UNAVAILABLE 收据跳过新纪元的强制发现
4. 后台健康检查每 300s 对所有 UNVERIFIED 条目和即将过期的 VERIFIED 条目执行完整发现
5. 历史存档收据仅作审计追溯，不等同于当前纪元有效收据
...
- Anthropic 和 Google 模型的可用性：历史上在 OpenRouter 层面被 Provider Allowlist 拦截（403），但该历史观察不属于当前发现纪元的运行时能力收据。实现阶段必须对两个 provider 执行强制发现，由运行时收据决定当前纪元状态（VERIFIED 或 UNAVAILABLE），不预判运行时结果
...
| 不可用验证 | 对每个 provider 生成当前纪元能力收据；仅收据可证明的拒绝/不可用才标记 UNAVAILABLE | 实现 |
...
FILES_MODIFIED:
  - coordination/design/AI-AGENT/H1_MULTI_MODEL_ADAPTER_DESIGN.md（重写 §2 模型矩阵 + 新增 §5 错误分类体系 + 增强 §6 强制发现）
  - coordination/evidence/AI-AGENT/H1_DESIGN_EVIDENCE.md（更新以对齐设计变更）
KEY_CHANGES:
  1. §2 模型矩阵重构：定义 VERIFIED 7 项必需字段（timestamp/provider/model/capability/result/failure_class/evidence_hash）；
     所有条目标为 UNVERIFIED；仅当前纪元完整运行时收据可建立 UNAVAILABLE 并在该纪元排除
  2. §2.3 路由规则强化：仅 VERIFIED 条目可用于路由决策；UNVERIFIED 禁止路由
  3. §3.1 接口增强：新增 ErrorClass 枚举、VerifiedEntry dataclass；AdapterCapability 新增 verified_entry 字段
  4. §4.1 回退触发条件：引用 §5 ErrorClass 分类体系
  5. 新增 §5 操作错误分类体系：7 个错误类（AUTH_FAILED/PROVIDER_DENIED/RATE_LIMITED/
     TRANSIENT_TRANSPORT/TIMEOUT/INVALID_RESPONSE/UNKNOWN），每类含回退资格、NO_TRADE 行为、
     有界重试预算、对账要求、已脱敏审计字段
  6. §5.9 分类汇总矩阵：一目了然的分类属性对比
  7. §6 能力发现协议增强：强制运行时启动发现（mandatory startup discovery）；
     无 VERIFIED = fail-closed；过期能力自动重验证；10s 超时保护
  8. §8.1 包结构调整：新增 errors/taxonomy.py
  9. §8.3 配置文件新增：error_taxonomy 参数段
  10. §9 测试策略：新增 ErrorClass 测试、强制发现测试、审计字段测试
UNRESOLVED_RISKS:
  ...
  - OpenRouter Provider Allowlist 未来变化可能影响回退可用性
