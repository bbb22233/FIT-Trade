...
- 客户端在提交确认时携带此 nonce。
- 服务端去重依赖计划中的原子确认消费（compare-and-consume，详见第 6 节），以 `confirmation_id`（UUID）为键。
  - **当前状态：NOT_IMPLEMENTED**。现有 `IdempotencyStore`（`services/trading-core/domain/idempotency.go`）仅提供基于 `client_order_id` 的订单级幂等去重，不包含 confirmation_id 键，也未实现 compare-and-consume 语义。
  - 客户端 UI 不得假定服务端已实现确认级去重。今日客户端仅可展示确认状态并提交确认请求，执行保持 fail closed。
...
2. 在多个设备/标签页打开同一确认卡 → **计划通过服务端 confirmation_id compare-and-consume 幂等去重实现（NOT_IMPLEMENTED）**。当前阶段客户端不得依赖此服务端去重：服务端原子确认消费（详见第 6 节）尚未实现；现有 `IdempotencyStore` 仅基于 `client_order_id` 去重，不基于 confirmation_id 去重。跨设备重复确认防护在 compare-and-consume 实现前处于 fail closed（无法保证去重）。
3. 网络断开后重连，UI 恢复时重新点击 → **按钮状态由服务端 state 驱动，不依赖客户端内存（依赖服务端 compare-and-consume 持久化状态，NOT_IMPLEMENTED）**。当前阶段重连后按钮状态完整性依赖于服务端 compare-and-consume 实现。
