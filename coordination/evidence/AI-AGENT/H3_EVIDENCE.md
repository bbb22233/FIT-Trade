...
The R2 baseline, its 0-commit and 12-uncommitted-file observations, are
**HISTORICAL_ARTIFACT_NOT_CURRENT_EVIDENCE**: it is not an ancestor of the
source candidate. The source candidate has direct parent
60850b69dd06c46bbe2a9cfab19d27cdb2ef0412 and 15 added AI-AGENT files. This
uncommitted R3 worktree cannot self-certify a fixed candidate;
candidate_binding=EXTERNAL_FIXED_OBJECT_REVIEW_REQUIRED.

## Fixed inputs

- Repository base commit: `8fcdb82db970a1e2925b53b61c4245f9b597a2ef`
...
| Idempotency/duplicate submission resistance | Sections 5.1–5.4: client-side button lock, confirmation_nonce, planned confirmation-level compare-and-consume (NOT_IMPLEMENTED / NOT_EXECUTED; existing IdempotencyStore only handles client_order_id), client_order_id conflict handling, reconnection reconciliation dependent on the planned store |
